...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let angle;
  let dotProd = x1*x2 + y1*y2;
  let modU = Math.sqrt(x1*x1 + y1*y1);
  let modV = Math.sqrt(x2*x2 + y2*y2);
  angle = Math.acos(dotProd/(modU*modV));
  return angle;
}
...
function roundToPowerOfTen(num, pow) {
    ...
    if(pow == 0)
    {
      ...
      return rem <= 5 ? (num - num%Math.pow(10,pow)) : (num + (Math.pow(10,pow) - num%Math.pow(10,pow)));
    }
}
...
function toNumber(value, def) {
    return (value != null) && (value instanceof Number || !isNaN(value-1) ) ? value : def;
}
